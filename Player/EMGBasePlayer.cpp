...
void AEMGBasePlayer::GUIMenus()
{
...
#ifdef _DEBUG_MOVEMENT
	{
...
#endif // _DEBUG_MOVEMENT

#ifdef _DEBUG_INVENTORY
		...
		ImGui::End();
#endif // _DEBUG_INVENTORY

#ifdef _DEBUG_POWERUP
...
void AEMGBasePlayer::CheckoutSingleItem_Implementation()
{
	IEMGPlayerInterface::CheckoutSingleItem_Implementation();
	...
	if (InventoryComp->PlayerInventory.Num() >= 1)
	{
//			player->GetPlayerState()->SetScore(newPlayerScore);
		if (AEMGPlayerState* ps = Cast<AEMGPlayerState>(GetPlayerState()))
		{
			int newPlayerScore = ps->GetScore() + InventoryComp->PlayerInventory.Top().Value;
			ps->SetScore(newPlayerScore);
		}
		InventoryComp->Server_RemoveLastItem();
	}
}
...
void AEMGBasePlayer::CompletedCheckingOut_Implementation()
{
	...
	if (AEMGPlayerState* ps = Cast<AEMGPlayerState>(GetPlayerState()))
	{
		ps->SetScore(CurrentScore + TotalItemsValue);
	}
